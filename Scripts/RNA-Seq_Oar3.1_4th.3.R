...
map_dfc(colnames(pheno), ~ as.factor(pheno2[,.]))
...
met <- data.frame( IllSeqSheep = c(paste("HF_San_", rep(1:3,1), sep = "" ), c("HF_Shall_1","LF_Shall_1","LF_SHall_2","HF_Shall_2","HF_SHall_3","LF_Shall_3")))
...
# Define models
models <- makeModels(sampleDepths, testvars = pheno[, c('Fecun')],
                     adjustvars = pheno$FecunBreed) # Use the same models for all your chromosomes unless you have a specific reason to use chromosome-specific models. 

##  Step4-2 Find candidate DERs

# Create a analysis directory
setwd(file.path("/home/MPourjam/RNA-Seq/RProject/RNA-Seq_Oar3.1_4th"))
dir.create('analysisResults2')
originalWd <- getwd()
setwd(file.path(originalWd,'analysisResults2'))
...
if (any(grepl("^results\\.RData$", dir(SavedData)))){
  ...
  system.time(results <- map(seq_along(filteredCov), ~ analyzeChr(chr = names(filteredCov[.]), coverageInfo = filteredCov[[.]], models,
                                                                  groupInfo = pheno$Fecun, writeOutput = TRUE, cutoffFstat = 5e-02, cutoffType = "theoretical",
                                                                  nPermute = 20, seeds = 20190114 + seq_len(20), returnOutput = TRUE, txdb = TxDb.Oaries.BioMart.ENSEMBLMARTENSEMBL.Oarv3.1,
...
mergeResults(chrs=c(1:26,"X"), prefix="analysisResults2",
             genomicState = genomicState$fullGenome, 
             optionsStats = results$`1`$optionsStats)

# Files created by mergeResults()
dir('analysisResults2', pattern = '.Rdata')

# fullRegions ---> The main result from mergeResults() is in fullRegions
load(file.path('analysisResults2', 'fullRegions.Rdata'))
seqinfo(fullRegions) <- OarSeqinfo

# fullAnotatedRegions
load(file.path('analysisResults2', 'fullAnnotatedRegions.Rdata'))
