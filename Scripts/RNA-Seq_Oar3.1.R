...
TxDb.Oaries.BioMart.ENSEMBLMARTENSEMBL.Oarv3.1 <- makeTxDbFromBiomart(taxonomyId = 9940, port = 80, biomart = "ENSEMBL_MART_ENSEMBL"
...
genomicState <- makeGenomicState(txdb =  TxDb.Oaries.BioMart.ENSEMBLMARTENSEMBL.Oarv3.1
...
##  Step4-3 Finding the best Cuttoff value
...
if (!file.exists(file.path("/home/MPourjam/RNA-Seq/RProject/RNA-Seq_Oar3.1_V/SavedData/Deltas.RData"))) {
  Deltas <<-  map(str_extract_all(RegionMatFilesPath,"RegionMats_[[:alpha:]]*_[[:digit:]]*"), ~ .x)
for (g in seq_len(length(RegionMatFilesPath))) {
  load(file = RegionMatFilesPath[g])
  regionMatVar <- get(ls()[which(str_detect(ls(), "RegionMats_[[:alpha:]]*_[[:digit:]]*"))])
  ...
  rm(list = ls()[which(str_detect(ls(), "RegionMats_[[:alpha:]]*_[[:digit:]]*"))])
}
rm(g)
names(Deltas) <- str_replace(string =  list.files(path = "/home/MPourjam/RNA-Seq/RProject/RNA-Seq_Oar3.1_V/SavedData/RegionMats", full.names = FALSE),
                             pattern = "[[:alpha:]]*[[:punct:]]+[[:alpha:]]*[[:punct:]]+([[:digit:]]+)[[:punct:]]+[[:alpha:]]*", replacement = "\\1")
...
Deltas[["MCC"]] <- factor(Deltas[["MCC"]], levels = as.character(c(1:50)))
Deltas[["Chr"]] <- factor(Deltas[["Chr"]], levels = as.character(c(1:26, "X")))
...
IdealMCC <-  which(DeltaValMedian_Zero$IdealPoint)[1]
...
IdealMCCfileName <- str_replace(list.files(path = "/home/MPourjam/RNA-Seq/RProject/RNA-Seq_Oar3.1_V/SavedData/RegionMats", 
                                           full.names = FALSE)[which(str_detect(list.files(path = "/home/MPourjam/RNA-Seq/RProject/RNA-Seq_Oar3.1_V/SavedData/RegionMats",full.names = FALSE),
                                                                                as.character(IdealMCC)))], pattern = "[[:alpha:]]*[[:punct:]]+([[:alpha:]]*[[:punct:]]+[[:digit:]]+)[[:punct:]]+[[:alpha:]]*", 
                                replacement = "\\1")
...
load(file = paste0(SavedData, "RegionMats/", paste0("RegionMats_", IdealMCCfileName, ".RData")))
regionMat <-  get(ls()[which(str_detect(string = ls(),pattern = paste0("RegionMats_", IdealMCCfileName)))])
rm(list = ls()[which(str_detect(string = ls(),pattern = paste0("RegionMats_", IdealMCCfileName)))])
...
for (b in seq_along(names(models))) {
  if (!any(grepl("^fullRegions.Rdata$", dir(paste0(ResultsPath, "/", names(models)[b] ))))) {
    ...
    system.time( assign(paste0("Results_", as.character(names(models)[b]), "_", IdealMCCfileName) , map(seq_along(filteredCov), ~ analyzeChr(chr = names(filteredCov[.]), coverageInfo = filteredCov[[.]], models = models[[as.character(names(models)[b])]],
