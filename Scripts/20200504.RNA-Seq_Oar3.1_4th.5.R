...
SavedData <- file.path("/media/animalscience/My Passport/MPourjam/RProject/RNA-Seq_Oar3.1_V/SavedData/")
...
bamfilespath <- list.files(path = "/media/animalscience/My Passport/MPourjam/DATA/Analyze/Out/Sheep/", pattern = ".Aligned.out.bam", all.files = TRUE, full.names = TRUE, recursive = TRUE )
readsnames <- gsub(".*/(.*)_Aligned.out.bam", "\\1", bamfilespath)
names(bamfilespath) <- readsnames
Coordbamfilespath <- list.files(path = "/media/animalscience/My Passport/MPourjam/DATA/Analyze/Out/Sheep", pattern = "sortedByCoord.out.bam$", full.names = TRUE, recursive = TRUE)
