...
if (any(grepl("^regionMat\\.RData$", dir(SavedData)))) {
  ...
  regionMat <- regionMatrix(fullCov, cutoff = 5 , L = c(rep(150,3), rep(100,2),rep(150, 4)), chrsStyle = "Ensembl", species = "ovis_aries", currentStyle = "Ensembl")
...
reportSaving <- function(ChrNum = seq_along(seqlevels(txdb))){
  stopifnot(class(ChrNum) == "numeric")
Date <- Sys.Date()
