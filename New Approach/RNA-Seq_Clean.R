...
# Above, we will have two big data frames occupying the RAM. Is it possible to avoid copying??? #
...
for (w in list.dirs("~/RNA-Seq/RProject/RNA-Seq_Oar3.1_V/Results", recursive = FALSE)) {
  ...
  derfinderReport(prefix='.', browse=FALSE,
                  nBestRegions=100, makeBestClusters=TRUE, outdir='analysisResults',
...
########################################################################################################
setwd(dir = "~/RNA-Seq/RProject/RNA-Seq_Oar3.1_V/Results/Breedtrt_Fecuntrt/")
load("./fullRegions.Rdata")
load("./fullNullSummary.Rdata")
load("./fullAnnotatedRegions.Rdata")
...
plotOverview(regions = keepSeqlevels(fullRegions,value = 3 ,pruning.mode="coarse") , annotation = keepSeqlevels(annoRegs, value = 3,pruning.mode="coarse"),
             type = 'pval' )
...
plotRegionCoverage(regions = fullRegions, regionCoverage = regionCov, 
                   groupInfo = pheno$Breedtrt, nearestAnnotation = fullAnnotationDF, 
